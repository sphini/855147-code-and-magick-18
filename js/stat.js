...
var BAR_HEIGHT = 150;
var TEXT_HEIGHT = 16;
var BAR_Y = CLOUD_Y + PADDING * 2 + TEXT_HEIGHT * 2 + FONT_Y_GAP;
...
window.renderStatistics = function (ctx, players, times) {
  ...
  createText(ctx, 'Список результатов:', CLOUD_X + PADDING, CLOUD_Y + PADDING + TEXT_HEIGHT + FONT_Y_GAP);

  var maxTime = getMaxElement(times);

  for (var i = 0; i < players.length; i++) {
    createText(ctx, players[i], CLOUD_X + GAP + (GAP + BAR_WIDTH) * i, BAR_Y + BAR_HEIGHT + FONT_Y_GAP * 2);
    ...
    createText(ctx, Math.round(times[i]), CLOUD_X + GAP + (GAP + BAR_WIDTH) * i, BAR_Y + (BAR_HEIGHT - currentBarHeight) - (FONT_Y_GAP + TEXT_HEIGHT));
  }
};
